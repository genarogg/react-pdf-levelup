...
const eliminarDependencias = (basePath = process.cwd(), customDepsToKeep) => {
  const packageJsonPath = path.join(basePath, "package.json");
  const backupPath = path.join(basePath, "package.json.bak");
  ...
  // Lista blanca: solo estas dependencias se mantendrán (siempre debe venir el arreglo)
  const depsToKeep = customDepsToKeep;
  ...
  try {
    // 6. Compilar y eliminarDependencias
    execSync("npm publish --access public", { stdio: "inherit", cwd: basePath });
...
export default eliminarDependencias;