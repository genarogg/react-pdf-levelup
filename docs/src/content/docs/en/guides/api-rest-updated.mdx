...
The API offers two PDF generation modes, each with its own endpoint:

- **Single**: `/api/pdf/single`
- **Worker**: `/api/pdf/worker`

### Single Mode

- **Endpoint**: `/api/pdf/single`
- **Description**: Processes the request on the main thread. It is faster for simple PDFs, but it can block the server if many requests are received simultaneously.

### Worker Mode

- **Endpoint**: `/api/pdf/worker`
- **Description**: Uses a worker pool to generate the PDF in separate threads. It is ideal for applications with high traffic or that generate complex PDFs, as it does not block the main thread.

### Quick Endpoint Copy

```text
https://react-pdf-levelup.nimbux.cloud/api/pdf/single
```

```text
https://react-pdf-levelup.nimbux.cloud/api/pdf/worker
```

<CardGrid>
  <Card title="Single Mode" icon="rocket">
    Ideal for getting started quickly and for simple PDFs.
  </Card>
  <Card title="Worker Mode" icon="gear">
    Recommended for production and for complex or high-traffic PDFs.
  </Card>
</CardGrid>
...
To use the API, you must provide a `username` and `password` in the `credentials` object. These credentials are compared with the `CR_USERNAME` and `CR_PASSWORD` environment variables on the server.
...
- Data URLs (base64) technically work, but greatly increase payload size — **not recommended for large images.**
...
const ENDPOINT_API = "https://react-pdf-levelup.nimbux.cloud/api/pdf/worker";
...
```bash
curl -X POST https://react-pdf-levelup.nimbux.cloud/api/pdf/worker \
  -H "Content-Type: application/json" \
  -d '{"template":"BASE64_TSX","data":{"name":"Genaro Gonzalez"},"credentials":{"username":"test","password":"test"}}'
```
...
endpoint = "https://react-pdf-levelup.nimbux.cloud/api/pdf/worker"
payload = {
    "template": "BASE64_TSX",
...
var endpoint = "https://react-pdf-levelup.nimbux.cloud/api/pdf/worker";
...
String endpoint = "https://react-pdf-levelup.nimbux.cloud/api/pdf/worker";
String json = "{\"template\":\"BASE64_TSX\",\"data\":{\"name\":\"Genaro Gonzalez\"},\"credentials\":{\"username\":\"test\",\"password\":\"test\"}}";
